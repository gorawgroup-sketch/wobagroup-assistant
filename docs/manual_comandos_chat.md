...
- Al pedir "Elegir otra carpeta", el asistente pide la empresa y carpeta correctas por texto libre — pedido explícito de Carlos, tras un caso real: no tiene por qué saber de memoria toda la estructura de Drive. Puedes responder "muéstrame las carpetas de [empresa]" y las lista de verdad (nunca inventa nombres); y si la carpeta que quieres no existe todavía, puedes pedir "créala" para que la arme antes de archivar ahí. Solo crea cuando lo pides explícitamente — nunca por su cuenta ante un nombre que no encontró.
