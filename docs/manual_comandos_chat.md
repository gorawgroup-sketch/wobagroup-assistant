...
- **Si no encuentra ninguno**, antes de proponer crear también revisa si el cargo ya está en el banco real (no solo en Compras), en TODAS las cuentas de tesorería sin importar su moneda (puede caer en una cuenta en USD y no en la de EUR):
  ...
  - 💰 Ajustar monto — pedido explícito de Carlos, tras un caso real (una cuota de comunidad que se paga a medias con otra parte, así que el gasto real a registrar es solo una fracción de lo que dice la factura): pide el monto real por texto libre (ej. "251.30", o "la mitad") y reescala todas las líneas (misma proporción de IVA/retención, solo cambia la base) — nunca inventa el monto, si no entiende el texto vuelve a preguntar. **No consume la propuesta** — igual que "Enseñar regla"/"Crear alerta" en documentos, se puede combinar con cualquiera de los botones de arriba, en el orden que haga falta (ajustar el monto y DESPUÉS crear el gasto, por ejemplo).
  - ❌ Cancelar — no escribe nada.
  - **Cuando la factura/gasto vino de un correo real** (no de una foto por Telegram), aparecen tres botones adicionales — pedido explícito de Carlos, tras un caso real (INDUBUILDING LUARCA): "no solo registrar el gasto... deberíamos poder responder el mail o dejar como conocimiento o programar un recordatorio... puede ser 1 sola cosa o pueden ser varias". Ninguno de los tres consume la propuesta — se pueden combinar entre sí y con cualquiera de los botones de arriba, en el orden que haga falta:
    - ✉️ Responder correo — redacta y ofrece un borrador de respuesta (con los mismos botones Enviar/Editar/Cancelar de siempre) enhebrado en el hilo real del correo original.
    - 🧠 Guardar como conocimiento — relee el cuerpo completo del correo (no el extracto) y lo guarda en la base de conocimiento, pidiendo antes a qué empresa corresponde.
    - ✏️ Otras acciones (recordatorio, instrucciones...) — pide por texto libre cualquier otra instrucción sobre ese correo (ej. "prográmame un recordatorio para conciliar mañana") y la ejecuta con las mismas herramientas del asistente normal (proponer correo, proponer evento, programar acción futura...) — se pueden combinar varias instrucciones en el mismo mensaje.
