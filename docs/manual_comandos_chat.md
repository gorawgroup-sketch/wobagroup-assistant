...
- Cuando hay correos nuevos sin leer, se manda un solo aviso ("📬 Tienes N correo(s) nuevo(s) sin leer. Empezamos por el más antiguo:") y se muestra solo el más antiguo.
- Cada correo se resuelve en el chat antes de mostrar el siguiente — nunca se muestran dos a la vez. Un correo con varios adjuntos necesita resolver todos sus adjuntos antes de pasar al siguiente. Al resolverse, se marca como leído en Gmail de verdad (requiere el scope `gmail.modify` autorizado en Workspace Admin — ya confirmado funcionando).
