...
| Comando | Qué hace |
|---|---|
| `/revisarcorreo` (o `revisarcorreo`, o `revisamail`/`/revisamail`) | Dispara de inmediato una revisión de correos nuevos en `asistente@wobagroup.com`, sin esperar al siguiente pase programado. |
...
| "escala esto a development", "repórtaselo a development", "mándaselo al equipo de desarrollo" | En chat normal, lenguaje natural — solo cuando se pide explícitamente escalar, nunca por iniciativa propia | Pedido explícito de Carlos: "que desde el chat podamos escalar directamente... sin tener que copiar y pegar" — `escalar_a_development` prepara un reporte completo y autocontenido (qué está mal, qué debería pasar, y el/los caso(s) real(es) que lo motivaron con sus datos concretos) y lo muestra con botones (ver Sección 3) — sin que nadie tenga que transcribir nada a mano en una sesión de Claude Code aparte. El GitHub Issue real solo se crea al aprobar el botón (`core/github/client.ts`, mismo `GITHUB_TOKEN` de la autorrevisión nocturna, necesita además el permiso "Issues: Read and write"). No modifica código ni despliega nada — solo deja el reporte registrado para que development lo recoja. |
...
**Revisión de correo: pase automático y después uno a uno, del más antiguo al más nuevo:**
- Cada ejecución empieza por revisar todos los mensajes sin leer y resolver automáticamente solo los gastos cuya creación, soporte y conciliación puedan comprobarse por completo. Después ofrece continuar uno a uno con los mensajes que quedaron pendientes.
- Se dispara cada dos horas en días hábiles y dos veces al día los fines de semana, o inmediatamente al pedirlo por chat (ver comando `revisarcorreo` en la sección 1). Solo se publican dos informes automáticos consolidados al día para no llenar el chat; una ejecución manual sí informa su resultado en el momento.
- Gmail (`is:unread` real, por **mensaje individual**) es la fuente de verdad de qué falta revisar. Un mensaje solo pasa a leído cuando todas sus acciones terminaron y se verificaron; cualquier fallo, dato pendiente o decisión del operador lo conserva sin leer. Un mensaje nuevo dentro de un hilo ya revisado mantiene su propia identidad y se procesa por separado.
...
- Pedido explícito de Carlos: al resolver un correo, YA NO pasa directo al siguiente — manda "✅ Resuelto. Quedan N correo(s) más en la cola — ¿seguimos con el siguiente?" con un botón "▶️ Sí, siguiente", para que no se cruce con otras consultas que se estén haciendo en el mismo chat al mismo tiempo. Si no se presiona, la cola se queda esperando (sin nada "activo") hasta que se apruebe, se pida `revisarcorreo`, o llegue el siguiente pase programado — que vuelve a preguntar, nunca muestra el correo directo.
...
- Si pides `revisarcorreo` y ya hay un correo activo bloqueando el resto de la cola, el aviso trae un botón "🗑️ Descartar y liberar" — pedido explícito de Carlos, para el caso de "esto ya lo gestioné" (lo resolvió por su cuenta, fuera del chat): libera la cola sin esperar las 48h ni tener que encontrar el mensaje original, y marca el correo como leído en Gmail de verdad (bug real encontrado en vivo: sin esto, Gmail lo seguía contando como sin leer para siempre y el siguiente pase lo volvía a encolar solo, deshaciendo el descarte sin avisar).
- La misma acción también se puede pedir en texto libre en cualquier momento, sin buscar ningún botón — pedido explícito de Carlos: "asegúrate de que cuando algo se bloquee pueda continuar con la revisión de correos sin tener que venir a esta instancia". Dile "sáltate este correo", "este está atascado, sigue con el siguiente" o similar, y libera la cola igual (marca leído, ofrece seguir con el siguiente).
- Todos los días a las 23:30 (hora Madrid), corre una auto-auditoría de las compras registradas en Holded (WOBA/EWORKS/Footprint) en los últimos 2 días (no solo "hoy" — Carlos suele procesar una cola de correos atrasados, así que la fecha de la factura casi nunca es la de hoy) — pedido explícito de Carlos, tras varios errores reales la misma noche: "auto audítate tu trabajo diariamente... para disminuir errores". Revisa 4 cosas, ninguna corrige nada por sí sola, solo avisa: (1) posibles gastos duplicados (mismo proveedor y monto el mismo rango de días); (2) el nombre del contacto asignado no aparece por ningún lado en la descripción real del gasto (el patrón del caso GoTo facturado a nombre de LinkedIn — señal, no prueba, puede tener falsos positivos con registros de prueba/genéricos); (3) una conversión de moneda (cuando el comprobante original venía en otra moneda) que se aleja más de un 4% de la tasa de cambio real del Banco Central Europeo ese día (consultada en vivo, nunca inventada); (4) una compra en moneda distinta a EUR con saldo pendiente de pago tras un pago parcial — el patrón del caso Salesmate/RapidOps, donde Holded aplicó el equivalente en EUR como si fuera el pago real y dejó un pendiente ficticio (ver reconciliarMovimiento en core/holded/write.ts, que ya detecta esto mismo en el momento de conciliar). Silencio total si no encuentra nada.
- Todos los días a las 19:10 (hora Madrid), cada admin/superadmin recibe un resumen de TODO lo que le sigue quedando pendiente sin resolver — no solo la cola de correo: propuestas de archivo, contactos por crear, gastos con datos faltantes, reclasificaciones de documento, capturas sin confirmar, alertas de documento, correcciones de gasto, desambiguaciones, ediciones de borrador, montos de pago recurrente, orientaciones de anotación/correo, y reglas de clasificación — cada línea numerada, con hace cuánto quedó pendiente. Silencio total si no hay nada (no manda "estás al día", solo avisa cuando de verdad falta algo). Cada línea descartable trae su propio botón "🗑️ Descartar #N" — pedido explícito de Carlos: poder descartar solo lo que ya no hace falta y seguir revisando el resto, en vez de solo tener la opción de todo o nada. Las preguntas "🤖 Falta decidir si es automática la conversación…" también traen su botón: descartarlas equivale a "❌ No, normal" (el hilo sigue en el flujo normal de correo, nunca se autoriza un envío automático). Además hay un botón "🧹 Limpiar pendientes" (pide confirmación) que las descarta todas de golpe, incluidas esas conversaciones (menos las propuestas de gasto, los contactos pendientes con una factura completa adentro, las ediciones de Holded sin aprobar, y los gastos con datos parciales — esos, por ser dinero real, siempre se revisan uno por uno, nunca en bloque ni individualmente desde acá). Los correos retirados de la cola siguen SIN LEER en Gmail, así que vuelven a revisarse.
...
- Al pedir "Elegir otra carpeta", el asistente pide la empresa y carpeta correctas por texto libre — pedido explícito de Carlos, tras un caso real: no tiene por qué saber de memoria toda la estructura de Drive. Puedes responder "muéstrame las carpetas de [empresa]" y las lista de verdad (nunca inventa nombres); si la carpeta que quieres no existe todavía, puedes pedir "créala" para que la arme antes de archivar ahí (solo crea cuando lo pides explícitamente, nunca por su cuenta ante un nombre que no encontró); y si en realidad no hace falta archivar nada, puedes decir "descártalo".
...
- **Todos los botones de una propuesta de gasto son checks ☐/✅, no acciones inmediatas** — pedido explícito de Carlos, tras probar la primera versión (donde cada botón actuaba al toque): "al presionar una [alternativa] ya se elimina la posibilidad de irse por otra... deberás dejar la posibilidad de activar una o varias y luego aprobar". Tocar un check solo lo marca/desmarca (☐↔✅) y repinta el MISMO mensaje — no dispara nada todavía. Solo **▶️ Aprobar selección** ejecuta TODO lo marcado, junto, en este orden fijo (para que una decisión final siempre use los datos ya corregidos, nunca los originales):
  1. Lo que no necesita texto (Responder correo, Guardar como conocimiento) se dispara de inmediato.
  2. Lo que sí necesita texto (Corregir clasificación, Ajustar monto, Otras acciones) se pregunta de a una, en ese orden — nunca se reparte un solo mensaje entre varias preguntas.
  3. La decisión final marcada (Crear/Crear y conciliar/Cancelar/Adjuntar a un candidato/Ninguno-crear nuevo) se dispara al final, cuando ya no falta ninguna respuesta de texto.
  - Marcar solo una es válido — se puede combinar una sola cosa o varias, la selección no se pierde entre check y check. Nada se ejecuta hasta pulsar "▶️ Aprobar selección".
  - **El resultado siempre aparece al final del chat** — caso real de Carlos (22/09, "¿está bloqueado el chat?"): tras "🔄 Aplicando tu selección..." el resultado ("❌ Cancelado — AEAT") se escribía en la propuesta original, arriba y fuera de vista. Ahora ese mismo mensaje "🔄 Aplicando..." se convierte en el resultado (o en el aviso de error); si la propuesta sigue abierta, sus botones vuelven en un mensaje nuevo al final. Nunca queda un "Aplicando..." abierto.
- **Una instrucción ya ejecutada nunca se vuelve a dejar pendiente** — caso real de Carlos (22/09, correo de Hacienda B.A.E.): tras ejecutar su instrucción, un fallo posterior decía "puedes reintentar" y dejaba armada la instrucción, así que su siguiente mensaje cualquiera la habría repetido (otro borrador, otro recordatorio). Ahora, si algo falla DESPUÉS de ejecutar (texto libre de orientación de correo, "Otras acciones", ajuste de monto, respuestas de la selección), WOBI avisa "⚠️ Tu instrucción ya se ejecutó..." con la causa, NO la re-arma, y ofrece «🔎 Verificar resultado» (solo lecturas). En el botón "✅ Proceder" de un correo, tras un fallo posterior a la ejecución se retira "Proceder" (quedan Descartar / Dar instrucciones) para que no se repita.
- **Si encuentra un gasto ya registrado en Holded que podría corresponder** (mismo proveedor — tolera razón social vs. nombre comercial — monto y fecha cercana): checks "✅ Es este (#N)" por cada uno (adjunta el comprobante a ese gasto, evita duplicarlo) o "❌ Ninguno, crear nuevo" (mutuamente excluyentes entre sí — marcar uno desmarca los demás), más "💰 Ajustar monto" y, si vino de un correo, los tres de abajo. Pedido explícito de Carlos: proveedor+monto+fecha cercana solos no bastan para distinguir "es el mismo gasto" de "son dos gastos reales distintos por el mismo importe" (ej. dos taxis del mismo proveedor y tarifa en días seguidos) — si la factura y algún candidato traen número de documento/comprobante, el mensaje también dice si coincide o es distinto, con un aviso adicional cuando eso cambia la lectura del caso. Es solo información de apoyo (un número mal leído por OCR puede engañar en cualquier dirección) — la decisión de qué botón tocar siempre la confirmas tú.
- **Si ya te había mandado una propuesta de "Crear gasto" para el mismo proveedor/monto que sigue sin resolver**, no manda una segunda — te avisa que revises la que ya tienes pendiente antes, para no arriesgar un duplicado. Si el número de documento de la factura nueva es DISTINTO al de esa propuesta pendiente, el aviso lo dice explícitamente (probablemente sean dos gastos reales distintos) — resuelve la pendiente primero y, si de verdad es un gasto aparte, dilo explícitamente para que se registre.
- **Si no encuentra ninguno**, antes de proponer crear también revisa si el cargo ya está en el banco real (no solo en Compras), en TODAS las cuentas de tesorería sin importar su moneda (puede caer en una cuenta en USD y no en la de EUR):
  - Si encuentra un movimiento bancario sin conciliar que coincide en monto y fecha, aparecen **los dos checks juntos**: ✅ Crear, y ✅ Crear y conciliar (hace ambas cosas de una) — mutuamente excluyentes, eliges tú según tu confianza en el match. Cuando el gasto original no está en euros, la comparación tolera unos céntimos de diferencia (el monto en EUR de la factura y el que calcula Holded para el movimiento vienen de conversiones de cambio independientes).
  - Si no hay match bancario, solo aparece ✅ Crear gasto en Holded — y la propuesta lo dice explícitamente ("no encontré ningún movimiento que coincida"), nunca se queda callada al respecto. Si en cambio hay **varios** movimientos parecidos y no sabe cuál es el correcto, aparece un check por cada uno ("🔗 Conciliar con #N", numerados igual que en el mensaje) más "✅ Crear (sin conciliar)" — pedido explícito de Carlos, tras un caso real donde la única forma de elegir era responder en texto libre, incompatible con marcar los demás checks en el mismo paso ("no tengo la posibilidad de darte las dos respuestas al mismo tiempo"): marca cuál conciliar y aprueba junto con cualquier otra acción, todo en la misma selección.
  - **Antes de rendirse del todo**, si no encontró nada en la moneda que trae la factura/correo, prueba el mismo número exacto en las OTRAS monedas reales de la empresa — pedido explícito de Carlos, tras un caso real (un correo reportó "$12.71" para Uber Eats, pero el cargo real había sido de 12.71 €, el remitente se equivocó de moneda). Si encuentra ese mismo número exacto en otra moneda, te avisa explícitamente ANTES de crear nada ("posible error de moneda") — nunca cambia la moneda solo, ni ofrece "Crear y conciliar" automáticamente para ese caso; confirma tú primero, y si hace falta, usa "💰 Ajustar monto" para decirle cuál es la moneda real (ver más abajo).
  - Crear (con o sin conciliar) registra el gasto (como borrador) con el IVA real de cada línea, la **cuenta contable correcta** (reutiliza una cuenta ya en uso real para proveedores/conceptos parecidos, ej. "Gastos de viaje" — nunca la cuenta genérica de Holded por defecto, ni una inventada; si no encuentra ninguna categoría parecida, te lo dice y te pregunta si tú sabes cuál debería ser) y adjunta el comprobante.
  - ✏️ Corregir clasificación — al aprobar, pide la empresa/concepto correctos por texto libre (ej. "EWORKS, servicio de limpieza") y los actualiza en la propuesta — YA NO crea el gasto por sí sola: se puede combinar con "Ajustar monto" y con la decisión final (Crear/Crear y conciliar), que se dispara DESPUÉS, usando la clasificación ya corregida. Aprender la clasificación para la próxima factura del mismo proveedor sigue pasando al crear el gasto.
  - 💰 Ajustar monto — pedido explícito de Carlos, tras un caso real (una cuota de comunidad que se paga a medias con otra parte, así que el gasto real a registrar es solo una fracción de lo que dice la factura): al aprobar, pide el monto real por texto libre (ej. "251.30", o "la mitad") y reescala todas las líneas (misma proporción de IVA/retención, solo cambia la base) — nunca inventa el monto, si no entiende el texto vuelve a preguntar. También reconoce cuando lo que está mal es la MONEDA, no el monto — pedido explícito de Carlos, tras un caso real (un correo reportó un gasto de Uber Eats como "$12.71" pero el cargo real había sido en euros): dile algo como "12.71 euros, la moneda estaba equivocada" y corrige moneda (y monto si también cambia), vuelve a buscar un movimiento bancario real con la moneda ya corregida, y actualiza los botones del mensaje original según lo que encuentre: "Crear y conciliar" si hay un match único, un check "🔗 Conciliar con #N" por cada uno si hay varios parecidos, o ninguno de los dos si sigue sin haber match. La moneda que digas se valida contra las cuentas reales de la empresa — si no tiene cuenta en esa moneda, te lo dice y pregunta de nuevo en vez de aceptarla a ciegas.
  - ❌ Cancelar — no escribe nada (mutuamente excluyente con Crear/Crear y conciliar).
  - **Cuando la factura/gasto vino de un correo real** (no de una foto por Telegram), aparecen tres checks adicionales — pedido explícito de Carlos, tras un caso real (INDUBUILDING LUARCA): "no solo registrar el gasto... deberíamos poder responder el mail o dejar como conocimiento o programar un recordatorio... puede ser 1 sola cosa o pueden ser varias":
    - ✉️ Responder correo — al aprobar, redacta y ofrece un borrador de respuesta (con los mismos botones Enviar/Editar/Cancelar de siempre) enhebrado en el hilo real del correo original.
    - 🧠 Guardar como conocimiento — al aprobar, relee el cuerpo completo del correo (no el extracto) y lo guarda en la base de conocimiento, pidiendo antes a qué empresa corresponde.
    - ✏️ Otras acciones (recordatorio, instrucciones...) — al aprobar, pide por texto libre cualquier otra instrucción sobre ese correo (ej. "prográmame un recordatorio para conciliar mañana") y la ejecuta con las mismas herramientas del asistente normal (proponer correo, proponer evento, programar acción futura...) — se pueden combinar varias instrucciones en el mismo mensaje.
- **Si no encuentra el proveedor** ni por nombre parecido al crear, busca alternativas (contactos con nombre parecido, facturas con el mismo importe) y las ofrece por botones antes de rendirse. Si el proveedor real SÍ se identificó con confianza desde el documento, aparece **"🆕 Crear contacto nuevo: '[nombre]'"**: crea un contacto propio y real en Holded para ESE proveedor (nunca compartido con otro gasto) y sigue creando el gasto contra él — a diferencia del genérico, SÍ aprende el alias, así que la próxima factura del mismo proveedor resuelve directo. Pedido explícito de Carlos tras un caso real: el contacto genérico compartido, si alguien lo renombra en Holded pensando que corrige un gasto puntual, cambia el nombre mostrado en TODOS los demás gastos que lo usan (pasados y futuros) — ya pasó 3 veces. En cualquier caso (con o sin alternativas) aparece además **"🆗 Crear sin proveedor real"**: registra el gasto ya mismo con el contacto genérico compartido ("PROVEEDOR SIN IDENTIFICAR", uno por empresa — la API de Holded exige siempre algún contacto, no acepta crear una compra sin ninguno), dejando el nombre real del proveedor al frente de la descripción del gasto (nunca se pierde) para corregir el contacto a mano en Holded cuando quieras — pero **nunca renombres ese contacto genérico directamente en Holded**, usa en su lugar "Crear contacto nuevo" la próxima vez que aparezca este mismo proveedor. Útil cuando de verdad no se pudo identificar ningún nombre real, o cuando lo urgente es conciliar ya mismo. Ambos botones requieren aprobación del superadministrador, igual que crear/adjuntar un gasto normal — solo "Crear sin proveedor real" evita aprender el alias (si lo aprendiera, la próxima factura del mismo proveedor seguiría cayendo en el genérico para siempre, incluso después de crear el contacto real).
- Solo el superadministrador puede pulsar **"▶️ Aprobar selección"** — un colaborador o admin puede marcar los checks que quiera, pero recibe el mensaje de que necesita aprobación al intentar aprobar. Marcar/desmarcar checks (☐↔✅) no requiere superadmin, ninguno escribe nada por sí solo.
- **Conciliación**: con "✅ Crear y conciliar" o "✅ Es este" ocurre en el mismo paso; con "✅ Crear" (solo crear) llega DESPUÉS, en un mensaje aparte, preguntando "¿quieres que intente conciliar el movimiento bancario?" con botones Sí/No — así puedes revisar el gasto recién creado en Holded antes de decidir. En todos los casos, ENLAZA de verdad el movimiento con el documento (nunca confía en que el estado diga "conciliado" sin confirmar que el monto quedó realmente emparejado). Si al responder "Sí, conciliar" (o al adjuntar un comprobante a un gasto ya existente) la búsqueda encuentra **varios** movimientos parecidos, no se queda en un texto sin salida — manda aparte un check "🔗 Conciliar con #N" por cada uno (más "❌ Ninguno, dejar así"), igual que en la propuesta original: marca cuál y listo, sin tener que revisarlo a mano en Holded. Nunca concilia movimientos huérfanos sin un gasto real detrás (para eso está la consulta de "movimientos sin conciliar", arriba).
- **Memoria de conciliaciones verificadas**: cada conciliación que Holded confirma como aplicada y realmente enlazada conserva empresa, proveedor, moneda, cuenta, descripción bancaria, último monto y número de confirmaciones. Esa evidencia se reutiliza para destacar con ⭐ el candidato más probable la próxima vez. Una propuesta, un timeout incierto o una coincidencia aproximada todavía no aprobada jamás alimentan la memoria; cuando hay empate o ambigüedad, el operador sigue eligiendo y el sistema no concilia solo.
...
- Pedido explícito de Carlos, tras un caso real (un gasto de McDonald's ya creado con el monto y el número de documento mal, sin ninguna forma de corregirlo salvo a mano en Holded o borrar y recrear desde cero): pídelo en texto libre (ej. "corrige el gasto de McDonald's, el monto real es 4,10€" o "el número de documento del gasto de OUIGO debería ser H5R3KL") — busca la compra por proveedor (y opcionalmente el monto actual, para acotar si hay varias) y, si encuentra exactamente una, propone la edición con botones. Si encuentra varias, lista cada una (id, número de documento actual, fecha, monto) y pregunta cuál es antes de proponer nada.
- Solo corrige **número de documento** y/o **monto total** — nunca proveedor, ni conceptos/líneas sueltos por texto libre (el endpoint de Holded reemplaza el documento COMPLETO en cada edición, así que cualquier corrección más profunda que eso se hace directo en la web de Holded).
- ✅ Confirmar edición — aplica el cambio EDITANDO el mismo documento real (mismo id, comprobante y cualquier pago/conciliación ya hecha contra él quedan intactos — nunca se borra y recrea). Relee Holded después de escribir para confirmar que el cambio realmente quedó (un "200 OK" de Holded no basta, dado que reemplaza el documento entero) — si algo no coincide con lo esperado, avisa explícitamente que la edición SÍ se envió y que hay que revisar en Holded, en vez de decir sin más que falló.
