...
| (automático, no se escribe) — correo con notas de reunión (ej. "Notes by Gemini" de Google Meet, Otter, Fireflies, Read.ai) | Detectado por el clasificador de correo entrante (`/revisarcorreo` o la revisión automática cada hora) | Lee el correo completo y dispara el mismo flujo de `CAPTURA` con botones de empresa — nunca se guarda hasta que alguien confirma. No depende de un remitente o asunto exacto (no se pudo verificar contra un correo real de Gemini todavía porque no ha llegado ninguno a la bandeja); si el clasificador falla en detectar uno real, avisa al administrador para ajustar la detección. |
