...
- Todos los días a las 19:10 (hora Madrid), cada admin/superadmin recibe un resumen de TODO lo que le sigue quedando pendiente sin resolver — no solo la cola de correo: propuestas de archivo, contactos por crear, gastos con datos faltantes, reclasificaciones de documento, capturas sin confirmar, alertas de documento, correcciones de gasto, desambiguaciones, ediciones de borrador, montos de pago recurrente, orientaciones de anotación/correo, y reglas de clasificación — cada línea numerada, con hace cuánto quedó pendiente. Silencio total si no hay nada (no manda "estás al día", solo avisa cuando de verdad falta algo). Cada línea descartable trae su propio botón "🗑️ Descartar #N" — pedido explícito de Carlos: poder descartar solo lo que ya no hace falta y seguir revisando el resto, en vez de solo tener la opción de todo o nada. Además hay un botón "🗑️ Descartar todo" para cuando de verdad no queda nada por revisar (menos las propuestas de gasto, los contactos pendientes con una factura completa adentro, las ediciones de Holded sin aprobar, y los gastos con datos parciales — esos, por ser dinero real, siempre se revisan uno por uno, nunca en bloque ni individualmente desde acá) y marca como leídos en Gmail los correos que se descarten.
