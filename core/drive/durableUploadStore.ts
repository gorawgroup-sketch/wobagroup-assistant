...
import type {
  EstadoSubidaDrive,
  RegistroSubidaDrive,
  RepositorioSubidasDrive,
  ResultadoSubidaDrive,
} from "./durableUpload";

const TAB_NAME = "_subidas_drive_durables";
const HEADERS = [
  "clave", "proceso", "estado", "marcador", "folderId", "driveFileId", "webViewLink", "creadoEn", "actualizadoEn", "verificadoEn",
];
const NUM_COLS = HEADERS.length;
const CLAVE_MUTEX = `drive-subidas:${TAB_NAME}`;
const RETENCION_MS = enteroAcotado(process.env.WOBI_DRIVE_LEDGER_RETENTION_DAYS, 180, 30, 365) * 24 * 60 * 60 * 1000;
const PURGA_CADA_MS = 6 * 60 * 60 * 1000;
const ESTADOS = new Set<EstadoSubidaDrive>(["preparada", "subiendo", "verificada", "incierta"]);

interface RegistroConFila extends RegistroSubidaDrive { rowIndex: number; }

export interface ResumenLedgerSubidasDrive {
  preparada: number;
  subiendo: number;
  verificada: number;
  incierta: number;
...
function desdeFila(rowIndex: number, valores: string[]): RegistroConFila | undefined {
  const estado = valores[2] as EstadoSubidaDrive;
  if (!valores[0] || !valores[3] || !valores[4] || !ESTADOS.has(estado)) return undefined;
  const verificadoEn = Number(valores[9]);
  return {
    rowIndex,
    clave: valores[0],
    proceso: valores[1] || "archivo_aprobado",
    estado,
    marcador: valores[3],
    folderId: valores[4],
    driveFileId: valores[5] || undefined,
    webViewLink: valores[6] || undefined,
...
function aFila(registro: RegistroSubidaDrive): (string | number)[] {
  return [
    registro.clave,
    registro.proceso,
    registro.estado,
...
class StoreSubidasDrive implements RepositorioSubidasDrive {
  ...
  marcarSubiendo(clave: string) {
    return this.cambiarEstado(clave, ["preparada"], "subiendo");
  }
  ...
  async marcarVerificada(clave: string, resultado: ResultadoSubidaDrive): Promise<void> {
    await conMutex(CLAVE_MUTEX, async () => {
      ...
      const siguiente: RegistroConFila = {
        ...actual,
        estado: "verificada",
        driveFileId: resultado.fileId || actual.driveFileId,
        webViewLink: resultado.webViewLink || actual.webViewLink,
        actualizadoEn: ahora,
        verificadoEn: ahora,
      };
  ...
  async listarPendientes(): Promise<RegistroSubidaDrive[]> {
    return conMutex(CLAVE_MUTEX, async () => {
      await this.inicializarYPurgar();
  ...
  async obtenerResumen(): Promise<ResumenLedgerSubidasDrive> {
    return conMutex(CLAVE_MUTEX, async () => {
      await this.inicializarYPurgar();
      const resumen: ResumenLedgerSubidasDrive = { preparada: 0, subiendo: 0, verificada: 0, incierta: 0 };
  ...
  private async cambiarEstado(
    clave: string,
    permitidos: EstadoSubidaDrive[],
    estado: EstadoSubidaDrive
  ): Promise<RegistroSubidaDrive | undefined> {
    return conMutex(CLAVE_MUTEX, async () => {
      await this.inicializarYPurgar();
      const actual = await this.refrescar(clave);
  ...
  private publico(registro: RegistroConFila): RegistroSubidaDrive {
    const { rowIndex: _rowIndex, ...publico } = registro;
    return { ...publico };
  }
}

export const durableUploadStore = new StoreSubidasDrive();
