...
const MUTEX_TRANSICIONES = "pendienteAccionGastoStore:transiciones";
...
export async function restaurarPendienteAccionGasto(pendiente: PendienteAccionGasto): Promise<void> {
  await conMutex(MUTEX_TRANSICIONES, async () => {
    const vigentes = await leerVigentes();
    ...
    if (delChat) throw new Error("Ya existe otra accion de gasto pendiente para este chat.");
