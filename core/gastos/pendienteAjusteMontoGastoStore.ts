...
const MUTEX_TRANSICIONES = "pendienteAjusteMontoGastoStore:transiciones";
...
export async function restaurarPendienteAjusteMontoGasto(pendiente: PendienteAjusteMontoGasto): Promise<void> {
  await conMutex(MUTEX_TRANSICIONES, async () => {
    const vigentes = await leerVigentes();
    ...
    if (delChat) throw new Error("Ya existe otro ajuste de monto pendiente para este chat.");
